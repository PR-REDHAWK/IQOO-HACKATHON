...
export const AnalyticsScreen = () => {
  ...
  const categoryColors = [
    { color: 'bg-secondary-container', text: 'text-secondary-container' },
    { color: 'bg-primary-fixed-dim', text: 'text-primary-fixed-dim' },
    { color: 'bg-error', text: 'text-error' },
    { color: 'bg-white', text: 'text-white' }
  ];
    ...
    <div className="min-h-screen bg-black text-[#e2e2e2] pt-20 pb-28 px-4 md:px-16 max-w-[1280px] mx-auto space-y-8">
      ...
      <div className="mt-4">
        ...
        <p className="text-xs text-on-surface-variant mt-0.5">
          {activeScenario?.description || 'Review historical expenditure trends, category distributions, and neural rules.'}
        </p>
      </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Value Protected', value: `$${formatCurrency(totalProtectedAmount)}`, icon: 'shield', color: 'text-secondary-container' },
          { label: 'Pending Approvals', value: pendingTransactions.length, icon: 'pending_actions', color: 'text-primary-fixed-dim' },
          { label: 'Approved Requests', value: approvedCount, icon: 'check_circle', color: 'text-secondary-container' },
          { label: 'Blocked Requests', value: blockedCount, icon: 'block', color: 'text-error' }
        ].map((item) => (
          <div key={item.label} className="glass-card rounded-2xl p-5 flex items-center gap-4 border border-white/5">
            ...
            <div className="min-w-0">
              <p className="text-[10px] text-on-surface-variant uppercase font-bold tracking-wider leading-none mb-1.5 truncate">{item.label}</p>
              <h3 className="font-headline-md text-[20px] font-extrabold text-white leading-none truncate">{item.value}</h3>
            </div>
          </div>
        ))}
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 glass-card rounded-2xl p-6 border border-white/5 space-y-6">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-on-surface-variant font-medium">Current Scenario</span>
          </div>
                ...
                <div key={d.day} className="flex-1 flex flex-col items-center gap-2 group">
                  <div className="relative w-full flex justify-center">
                    ...
                    <div 
                      ...
                      style={{ height: `${Math.max(percentage * 1.5, d.value > 0 ? 8 : 0)}px` }}
                    ></div>
                  </div>
        ...
        <div className="lg:col-span-5 glass-card rounded-2xl p-6 border border-white/5 space-y-6">
          ...
          <div className="space-y-4 pt-2">
            {categoryData.length === 0 ? (
              <div className="text-center border border-dashed border-white/10 rounded-xl p-6 text-xs text-on-surface-variant">
                No blocked categories in this scenario.
              </div>
            ) : (
                ...
                return (
                  <div key={cat.category} className="space-y-1.5">
                    <div className="flex justify-between text-xs font-semibold">
                      ...
                      <span className={palette.text}>{cat.percentage}%</span>
                    </div>
                    <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                      <div className={`h-full ${palette.color} rounded-full`} style={{ width: `${cat.percentage}%` }}></div>
                    </div>
                  </div>
                );
              })
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
          <div className="space-y-1.5 bg-white/[0.02] border border-white/5 p-4 rounded-xl">
            ...
            <p className="text-on-surface-variant text-[12px] leading-relaxed">
              {topCategory
                ? `${topCategory.category} accounts for ${topCategory.percentage}% of blocked value in this scenario.`
                : 'No blocked category trend is active in this scenario.'}
            </p>
          </div>
          <div className="space-y-1.5 bg-white/[0.02] border border-white/5 p-4 rounded-xl">
            <h4 className="text-white font-bold flex items-center gap-1">
            ...
            <p className="text-on-surface-variant text-[12px] leading-relaxed">
              Current scenario risk peaks at {analytics.highestRiskScore}%, with ${formatCurrency(analytics.pendingAmount)} still awaiting approval.
            </p>
          </div>
        </div>
      </section>
